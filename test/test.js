...
describe("NFTMarket", function() {
  it("Should interact with the token contract", async function() {
    ...
    await market.createMarketItem(nftContractAddress, 1,1,1)
    await market.createMarketItem(nftContractAddress, 2,2.034,2)
    await market.createMarketItem(nftContractAddress, 3,1,3)
    
    const [_, userAddress, userAddress2, userAddress3] = await ethers.getSigners();

    // await market.connect(userAddress).createMarketSale(nftContractAddress, 1,0.21,1)
    // await market.connect(userAddress2).createMarketSale(nftContractAddress, 2,0.23,2)
    // await market.connect(userAddress2).createMarketSale(nftContractAddress, 3,0.54,3)
...
//     await market.connect(userAddress2).createMarketSale(nftContractAddress, 4) // d
